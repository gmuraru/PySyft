...
def test_reconstruct(clients: list) -> None:
...
def test_add(clients: list) -> None:
    alice_client, bob_client, james_client = clients
    session = SySession(parties=[alice_client, bob_client, james_client])
    session.setup_mpc()
    ...
    x = AdditiveSharingTensor(secret=x_secret, session=session)
    result = (x + c).reconstruct()
    assert torch.allclose(result, (x_secret + y))

    # test with different sessions
    session_alternate = sy.lib.sympc.session.SySession(
    ...
    x_session_alternate = AdditiveSharingTensor(
        secret=x_secret, session=session_alternate
    )
...
def test_sub(clients: list) -> None:
    alice_client, bob_client, james_client = clients
    session = SySession(parties=[alice_client, bob_client, james_client])
    session.setup_mpc()
